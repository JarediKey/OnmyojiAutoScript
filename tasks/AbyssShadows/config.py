...
class Condition:
    ...
    def __init__(self, value: str):
        ...
        if value not in ('TRUE', 'FALSE') and (not value.isascii() or not value.isdecimal()):
            raise ValueError('Invalid Abyss battle condition')
        # True 立即退出
...
class ProcessManage(ConfigBase):
    ...
    @field_validator('attack_order')
    @classmethod
    def validate_attack_order(cls, value: str) -> str:
        tokens = [token.strip().upper() for token in value.split(';') if token.strip()]
        import re
        if not tokens or any(re.fullmatch(r'[A-D](?:-[1-6])?|[1-6]', token) is None for token in tokens):
            raise ValueError('Use A-D, 1-6, or A-1 style targets separated by semicolons')
        return ';'.join(tokens)

    @field_validator('preset_boss', 'preset_general', 'preset_elite')
    @classmethod
    def validate_preset(cls, value: str) -> str:
        ...
        if not value:
            return ''
        import re
        match = re.fullmatch(r'([1-7])\s*,\s*([1-4])', value)
        if not match:
            raise ValueError('Use group,team (groups 1-7, teams 1-4), or empty to retain the current lineup')
        return ','.join(match.groups())

    @field_validator('strategy_boss', 'strategy_general', 'strategy_elite')
    @classmethod
    def validate_strategy(cls, value: str) -> str:
        ...
        if not value.isascii() or not value.isdecimal():
            raise ValueError('Use TRUE, FALSE, 1-3 digits for seconds, or 4+ digits for damage')
        return value
...
class SavedParams(ConfigBase):
    ...
    failed: str = Field(default='', description='abyss_failed_targets_help')
