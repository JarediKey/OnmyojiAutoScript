...
class battle_wait_options:
    ...
    def __enter__(self):
        ...
        self._previous_context = type(self)._context_overrides
        type(self)._context_overrides = {**self._previous_context, **self._overrides}
    ...
    def __exit__(self, *exc):
        ...
        type(self)._context_overrides = self._previous_context
        return False
    ...
    def __call__(self, func: Callable[P, T]) -> Callable[P, T]:
        ...
        @wraps(func)
        def inner(owner, *args: P.args, **kwargs: P.kwargs) -> T:
            previous = type(self).options
            options = {event: cls() for event, cls in _OPTION_CLASSES.items()}
            options.update(deepcopy(self._overrides))
            options.update(deepcopy(type(self)._context_overrides))
            type(self).options = options
            try:
                return func(owner, *args, **kwargs)
            finally:
                type(self).options = previous
...
class runtime:
    ...
    def update_options(cls, options: dict[str, object] | None, plan: BattleWaitPlan):
        ...
        if not isinstance(options, dict):
            raise TypeError("Battle options must be a dictionary")
